...
1. Upload the `wegame-tournoi` folder to `wp-content/plugins/`, or install the ZIP file from Plugins > Add New.
...
5. The tournament page is created automatically. You can also place the `[wegame_tournoi]` shortcode in any page.
...
Every shortcode except `[wegame_tournois]` accepts `tournoi="slug"` (alias `tournament`, a numeric ID also works). Without it, the shortcode shows the tournament of the current page, otherwise the default tournament of the site.

* `[wegame_tournoi]` - full tabbed page. Accepts `fit="width"`.
* `[wegame_tournois]` - list of every published tournament of the site.
* `[wegame_tableau]` - the bracket alone. `header="no"` hides the title banner; `fit="width"` scales to width only.
* `[wegame_poules]` - group standings.
* `[wegame_classement]` - overall ranking.
* `[wegame_planning]` - schedule.
* `[wegame_resultats]` - result sheet.
* `[wegame_equipes]` - registered teams. `players="yes"` shows the line-ups.
* `[wegame_reglement]` - rules.
* `[wegame_organisation]` - staff needed.
* `[wegame_checklist]` - checklist before opening.
* `[wegame_inscription]` - sign-up form. `simple="yes"` for a reduced form.

Example: `[wegame_tableau tournoi="we-game-2026" header="no" fit="width"]`

== REST API ==

Two public read-only routes, for a display screen or an external dashboard.

* `GET /wp-json/wegame/v1/state` - the full state of the tournament as JSON.
* `GET /wp-json/wegame/v1/render?view=bracket` - the HTML of one view.
...
* Reduced sign-up form: `[wegame_inscription simple="yes"]`.
