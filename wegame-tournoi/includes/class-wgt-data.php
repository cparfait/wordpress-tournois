...
class WGT_Data {
	...
	public static function recalculate( $tournament_id ) {
		...
		foreach ( WGT_Bracket::processing_order( $matches ) as $code ) {
			$match = $matches[ $code ];
			...
			$bye1 = WGT_Bracket::source_is_empty( $match['src1'], $matches );
			$bye2 = WGT_Bracket::source_is_empty( $match['src2'], $matches );
		...
		if ( 'G:' === substr( $src, 0, 2 ) ) {
			...
			if ( 3 === count( $parts ) ) {
				...
				if ( ! isset( $ctx['standings'][ $g ] ) ) {
					$ctx['standings'][ $g ] = WGT_Standings::qualified_table( $tournament_id, $g, array_values( $matches ), $ctx['teams'] );
				}
				$rank = (int) $parts[2] - 1;
	...
	protected static function structure_follows_teams( $tournament_id ) {
		$tid = (int) $tournament_id;
		return 'groups' === WGT_Tournament::get( $tid, 'format' ) && 'manual' === WGT_Tournament::get( $tid, 'group_mode' );
	}
	...
	public static function group_members( $tournament_id, $group_id ) {
		...
		$mode   = WGT_Tournament::get( $tid, 'group_mode' );
		...
		$layout = WGT_Bracket::group_layout( WGT_Tournament::settings( $tid ) );
		$seeds  = WGT_Bracket::group_seeds( $layout['count'], $layout['groups'] );
	...
	public static function is_match_needed( $match, $matches ) {
		...
		if ( isset( $match['src1'], $match['src2'] ) ) {
			if ( WGT_Bracket::source_is_empty( $match['src1'], $matches ) && WGT_Bracket::source_is_empty( $match['src2'], $matches ) ) {
				return false;
			}
		}
	...
	public static function get_stats( $tournament_id ) {
		...
		foreach ( $matches as $match ) {
			...
			$playable = WGT_Bracket::is_playable( $match, $map ) && self::is_match_needed( $match, $map );
			if ( ! $playable ) {
				$skipped++;
