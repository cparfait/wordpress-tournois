...
class WGT_Tournament {

	const POST_TYPE = 'wgt_tournament';
	const META      = '_wgt_settings';
	...
	public static function current_admin() {
		...
		if ( $user_id ) {
			$saved = (int) get_user_meta( $user_id, 'wgt_current_tournament', true );
	...
	public static function duplicate( $source_id, $title, $with_teams = false ) {
		...
		if ( $with_teams ) {
			foreach ( WGT_Data::get_teams( array( 'tournament_id' => $source_id ) ) as $team ) {
				WGT_Data::save_team(
					array(
	...
	protected static function purge_data( $id ) {
		...
		if ( (int) get_option( 'wgt_default_tournament' ) === $id ) {
			update_option( 'wgt_default_tournament', 0 );
		}
		delete_metadata( 'user', 0, 'wgt_current_tournament', $id, true );

		// Page d'inscription créée par l'extension : elle part avec le tournoi.
		$page_id = (int) get_post_meta( $id, '_wgt_registration_page', true );
	...
	public static function registration_page( $id ) {
		$page_id = (int) get_post_meta( (int) $id, '_wgt_registration_page', true );
	...
	public static function create_registration_page( $id, $status = 'publish' ) {
		...
		$page_id = wp_insert_post(
			array(
				...
				'post_content' => '[wegame_inscription simple="yes" tournoi="' . $slug . '"]',
			),
			true
		);
		...
		update_post_meta( $id, '_wgt_registration_page', (int) $page_id );
	...
	public static function append_to_content( $content ) {
		...
		if ( ! WGT_Tournament::get( $id, 'auto_content' ) ) {
			return $content;
		}
		...
		return $content . WGT_Render::view( 'full', array( 'tournament_id' => $id ) );
	}
}
