/* We Game Tournoi — script public (sans dépendance) */
( function () {
	'use strict';
	...
	function initTabs( root ) {
		var buttons = root.querySelectorAll( '[data-wgt-tab]' );
		var panels = root.querySelectorAll( '[data-wgt-panel]' );

		Array.prototype.forEach.call( buttons, function ( btn ) {
			btn.addEventListener( 'click', function () {
				var target = btn.getAttribute( 'data-wgt-tab' );
				...
				Array.prototype.forEach.call( panels, function ( p ) {
					p.classList.toggle( 'is-active', p.getAttribute( 'data-wgt-panel' ) === target );
				} );

				if ( window.history && window.history.replaceState ) {
					window.history.replaceState( null, '', '#wgt-' + target );
				}
			} );
		} );

		// Ouverture directe via l'ancre.
		var hash = window.location.hash.replace( '#wgt-', '' );
		if ( hash ) {
			var btn = root.querySelector( '[data-wgt-tab="' + hash.replace( /[^a-z]/g, '' ) + '"]' );
			if ( btn ) {
				btn.click();
			}
		}
	 ...
	 *
	 * @param {string} key      Clé dans WGT_CFG.i18n.
	 * @param {string} fallback Texte de repli.
	 * @return {string}
	 */
	function i18n( key, fallback ) {
		if ( typeof WGT_CFG !== 'undefined' && WGT_CFG.i18n && WGT_CFG.i18n[ key ] ) {
			return String( WGT_CFG.i18n[ key ] );
		}
		return fallback;
	}
	...
	function statusElement( node ) {
		var el = node.querySelector( '.wgt-refresh-status' );
		if ( ! el ) {
			el = document.createElement( 'p' );
			el.className = 'wgt-refresh-status';
	...
	function renderUrl( node ) {
		var view = node.getAttribute( 'data-wgt-live' );
		var tournament = node.getAttribute( 'data-wgt-tournament' ) || '';
		var header = node.getAttribute( 'data-wgt-header' ) === 'no' ? 'no' : 'yes';
		var fit = node.getAttribute( 'data-wgt-fit' ) === 'width' ? 'width' : 'screen';
		var url = WGT_CFG.endpoint + ( WGT_CFG.endpoint.indexOf( '?' ) === -1 ? '?' : '&' ) + 'view=' + encodeURIComponent( view );
	...
	function refreshNode( node ) {
		return fetch( renderUrl( node ), { credentials: 'same-origin' } )
			...
			.then( function ( data ) {
				...
				var status = node.querySelector( '.wgt-refresh-status' );
				if ( status ) {
	...
	/**
	 * Rafraîchissement automatique des vues marquées data-wgt-live.
	 *
	 ...
	 */
	function initLive() {
		if ( typeof WGT_CFG === 'undefined' || ! WGT_CFG.interval || WGT_CFG.interval < 10 ) {
			return;
		}

		var nodes = document.querySelectorAll( '[data-wgt-live]' );
		if ( ! nodes.length ) {
			return;
		}

		var base = Number( WGT_CFG.interval ) * 1000;
		var max = 5 * 60 * 1000;
		var current = base;
		var failures = 0;
	...
	function availableHeight( wrap ) {
		...
		while ( node ) {
			if ( node.classList && node.classList.contains( 'wgt' ) ) {
				view = node;
			}
	 ...
	 *
	 * Deux modes, lus sur data-wgt-fit-mode :
	...
	function fitBrackets() {
		Array.prototype.forEach.call( document.querySelectorAll( '[data-wgt-fit]' ), function ( wrap ) {
			var inner = wrap.querySelector( '.wgt-bracket-fit' );
			var bracket = wrap.querySelector( '.wgt-bracket' );
			...
			var widthOnly = 'width' === wrap.getAttribute( 'data-wgt-fit-mode' );
	...
	function ready() {
		Array.prototype.forEach.call( document.querySelectorAll( '[data-wgt-tabs]' ), initTabs );
		fitBrackets();
		initLive();
		...
		document.addEventListener( 'click', function ( event ) {
			if ( event.target.closest && event.target.closest( '[data-wgt-tab]' ) ) {
				scheduleFit();
			}
		} );
	}
	...
	window.wgtFitBrackets = fitBrackets;
