 ...
 * équipes il reproduit exactement le tableau du dossier Brackethive 2026
...
class Brackethive_Bracket {
	...
	public static function round_label( $key, $bo = 0 ) {
		$labels = array(
			'final' => __( 'Final', 'brackethive' ),
			'sf'    => __( 'Semi-finals', 'brackethive' ),
			'qf'    => __( 'Quarter-finals', 'brackethive' ),
			'r16'   => __( 'Round of 16', 'brackethive' ),
			'r32'   => __( 'Round of 32', 'brackethive' ),
			'r64'   => __( 'Round of 64', 'brackethive' ),
			'third' => __( 'Third place match', 'brackethive' ),
			'group' => __( 'Group stage', 'brackethive' ),
			'gf'    => __( 'Grand final', 'brackethive' ),
		);
		...
		if ( isset( $labels[ $key ] ) ) {
			...
			$label = sprintf( __( 'Losers bracket — round %d', 'brackethive' ), (int) substr( $key, 2 ) );
		} else {
			$label = $key;
		}
	...
	public static function round_short( $key ) {
		$labels = array(
			'final' => __( 'Final', 'brackethive' ),
			'sf'    => __( 'Semis', 'brackethive' ),
			'qf'    => __( 'Quarters', 'brackethive' ),
			'r16'   => __( 'R16', 'brackethive' ),
			'r32'   => __( 'R32', 'brackethive' ),
			'r64'   => __( 'R64', 'brackethive' ),
			'third' => __( 'Third place', 'brackethive' ),
			'group' => __( 'Groups', 'brackethive' ),
			'gf'    => __( 'Grand final', 'brackethive' ),
		);
		...
		if ( 0 === strpos( $key, 'lb' ) ) {
			/* translators: %d: numéro de tour de repêchage */
			return sprintf( __( 'Losers round %d', 'brackethive' ), (int) substr( $key, 2 ) );
		}
		return $key;
	}
	...
	public static function structure( $settings = array() ) {
		$settings = array_merge( Brackethive_Tournament::defaults(), is_array( $settings ) ? $settings : array() );
	...
	public static function staff() {
		return array(
			array( __( 'Tournament manager', 'brackethive' ), 1, __( 'Supervision, rules, decisions and coordination', 'brackethive' ) ),
			array( __( 'Front desk / management', 'brackethive' ), 1, __( 'Sign-ups, results, bracket and times', 'brackethive' ) ),
			array( __( 'Referees', 'brackethive' ), 2, __( 'Match follow-up, score approval, disputes', 'brackethive' ) ),
			array( __( 'Technicians', 'brackethive' ), 2, __( 'Stations, network, accounts, peripherals, incidents', 'brackethive' ) ),
			array( __( 'Host / communication', 'brackethive' ), 1, __( 'Announcements, atmosphere, hosting, prize ceremony', 'brackethive' ) ),
		);
	}
	...
	public static function rules( $settings = array() ) {
		...
		return array(
			...
			__( 'The captain is the team\'s official contact.', 'brackethive' ),
			__( 'Players must follow the instructions of the referees and the organizers.', 'brackethive' ),
			__( 'Any cheat, deliberate exploit or unsportsmanlike behavior may lead to a penalty or a disqualification.', 'brackethive' ),
			__( 'Technical problems must be reported to the referee immediately.', 'brackethive' ),
			__( 'Results are approved by the referee before moving on to the next round.', 'brackethive' ),
			__( 'The organizers may adjust the schedule in the event of a technical problem or a significant delay.', 'brackethive' ),
		);
	}
	...
	public static function checklist() {
		return array(
			__( 'Check the stations and peripherals', 'brackethive' ),
			__( 'Test the network and the connection to game services', 'brackethive' ),
			__( 'Check the accounts / profiles used', 'brackethive' ),
			__( 'Prepare the tournament bracket and the score sheets', 'brackethive' ),
			__( 'Identify the team captains', 'brackethive' ),
			__( 'Brief the players and referees', 'brackethive' ),
			__( 'Prepare prizes / trophy / rewards', 'brackethive' ),
			__( 'Plan for water, breaks and a waiting area', 'brackethive' ),
		);
	}
}
