...
class Brackethive_Theme {
	...
	public static function blank_title( $title, $id = 0 ) {
		if ( is_admin() || ! $id || ! Brackethive_Settings::get( 'hide_page_title' ) ) {
			return $title;
		}
	...
	public static function is_tournament_page() {
		...
		if ( Brackethive_Tournament::POST_TYPE === $post->post_type ) {
			return true;
		}
	...
	public static function body_class( $classes ) {
		...
		$classes[] = 'brackethive-has-tournament';

		if ( Brackethive_Settings::get( 'hide_page_title' ) ) {
			$classes[] = 'brackethive-hide-page-title';
		}
	...
	protected static function selectors() {
		...
		$extra = (string) Brackethive_Settings::get( 'hide_title_selector' );
		if ( '' !== $extra ) {
	...
	public static function inline_css() {
		if ( ! self::is_tournament_page() || ! Brackethive_Settings::get( 'hide_page_title' ) ) {
			return;
		}

		$parts = array();
		foreach ( self::selectors() as $selector ) {
			$parts[] = 'body.brackethive-hide-page-title ' . $selector;
		}
		...
		wp_add_inline_style( 'brackethive-public', $css );
	}
}
