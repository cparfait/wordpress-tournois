...
class Brackethive_IO {
	...
	protected static function flat_games( $tournament_id ) {
		$out = array();
		foreach ( Brackethive_Data::get_all_games( $tournament_id ) as $rows ) {
			foreach ( $rows as $row ) {
				$out[] = $row;
			}
		}
	...
	public static function filename( $tournament_id = 0 ) {
		...
		$slug = $slug ? $slug . '-' : '';
		return 'brackethive-' . $slug . gmdate( 'Y-m-d-Hi', (int) current_time( 'timestamp' ) ) . '.json'; // phpcs:ignore WordPress.DateTime.CurrentTimeTimestamp -- Horodatage local volontaire : le nom de fichier doit refléter l'heure du site.
	}
	...
	public static function import( $json, $tournament_id, $with_settings = false ) {
		...
		$tid = (int) $tournament_id;
		if ( ! $tid || ! Brackethive_Tournament::exists( $tid ) ) {
			return new WP_Error( 'brackethive_no_tournament', __( 'Target tournament not found.', 'brackethive' ) );
		}

		$data = json_decode( $json, true );

		if ( ! is_array( $data ) ) {
			return new WP_Error( 'brackethive_bad_json', __( 'Unreadable file: this is not valid JSON.', 'brackethive' ) );
		}

		if ( ! isset( $data['plugin'] ) || 'brackethive' !== $data['plugin'] ) {
			return new WP_Error( 'brackethive_bad_source', __( 'This file does not come from the Brackethive plugin.', 'brackethive' ) );
		}

		if ( ! isset( $data['format'] ) || (int) $data['format'] > self::FORMAT ) {
			return new WP_Error( 'brackethive_bad_format', __( 'This file was produced by a newer version of the plugin.', 'brackethive' ) );
		}

		if ( ! isset( $data['teams'], $data['matches'] ) || ! is_array( $data['teams'] ) || ! is_array( $data['matches'] ) ) {
			return new WP_Error( 'brackethive_incomplete', __( 'Incomplete backup: teams or matches are missing.', 'brackethive' ) );
		}
		...
		$teams_table   = brackethive_table( 'teams' );
		$matches_table = brackethive_table( 'matches' );
		$games_table   = brackethive_table( 'games' );
		...
		foreach ( $data['teams'] as $team ) {
			...
			if ( false === $wpdb->insert( $teams_table, self::clean_team( $team, $tid ) ) ) { // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Table propre à l'extension ; données transactionnelles non mises en cache.
				return self::fail( $wpdb, __( 'Restore aborted: a team could not be saved. No data was kept.', 'brackethive' ) );
			}
			if ( $old_id > 0 ) {
		...
		foreach ( $data['matches'] as $match ) {
			...
			if ( false === $wpdb->insert( $matches_table, self::clean_match( $match, $tid, $team_map ) ) ) { // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Table propre à l'extension ; données transactionnelles non mises en cache.
				return self::fail( $wpdb, __( 'Restore aborted: a match could not be saved. No data was kept.', 'brackethive' ) );
			}
			if ( $old_id > 0 ) {
		...
		foreach ( $games as $game ) {
			...
			if ( false === $wpdb->insert( $games_table, $row ) ) { // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Table propre à l'extension ; données transactionnelles non mises en cache.
				return self::fail( $wpdb, __( 'Restore aborted: a game could not be saved. No data was kept.', 'brackethive' ) );
			}
		}
		...
		if ( $with_settings && isset( $data['tournament']['settings'] ) && is_array( $data['tournament']['settings'] ) ) {
			Brackethive_Tournament::save_settings( $tid, $data['tournament']['settings'] );
		}
	...
	protected static function fail( $wpdb, $message ) {
		...
		return new WP_Error( 'brackethive_import_failed', $message . $detail );
	}
	...
	protected static function validate_rows( $teams, $matches, $games ) {
		foreach ( $teams as $i => $team ) {
			if ( ! is_array( $team ) ) {
				return new WP_Error( 'brackethive_bad_team', sprintf( /* translators: %d: index */ __( 'Team no. %d is unreadable.', 'brackethive' ), (int) $i + 1 ) );
			}
			if ( ! isset( $team['name'] ) || ! is_scalar( $team['name'] ) || '' === trim( (string) $team['name'] ) ) {
				return new WP_Error( 'brackethive_bad_team', sprintf( /* translators: %d: index */ __( 'Team no. %d has no name.', 'brackethive' ), (int) $i + 1 ) );
			}
		}
		...
		foreach ( $matches as $i => $match ) {
			if ( ! is_array( $match ) ) {
				return new WP_Error( 'brackethive_bad_match', sprintf( /* translators: %d: index */ __( 'Match no. %d is unreadable.', 'brackethive' ), (int) $i + 1 ) );
			}
			$code = isset( $match['code'] ) && is_scalar( $match['code'] ) ? sanitize_text_field( (string) $match['code'] ) : '';
			if ( '' === $code ) {
				return new WP_Error( 'brackethive_bad_match', sprintf( /* translators: %d: index */ __( 'Match no. %d has no code.', 'brackethive' ), (int) $i + 1 ) );
			}
			if ( isset( $codes[ $code ] ) ) {
				return new WP_Error( 'brackethive_bad_match', sprintf( /* translators: %s: code du match */ __( 'Duplicate match code "%s".', 'brackethive' ), $code ) );
			}
			$codes[ $code ] = true;
		}
		...
		foreach ( $games as $i => $game ) {
			if ( ! is_array( $game ) ) {
				return new WP_Error( 'brackethive_bad_game', sprintf( /* translators: %d: index */ __( 'Game no. %d is unreadable.', 'brackethive' ), (int) $i + 1 ) );
			}
		}
	...
	/**
	 * Nettoie une ligne d'équipe (colonnes de brackethive_teams, sans id).
	 *
	...
	/**
	 * Nettoie une ligne de match (colonnes de brackethive_matches, sans id).
	 *
	 * @param array $row      Ligne brute.
	 * @param int   $tid      Tournoi de destination.
	...
	/**
	 * Nettoie une ligne de manche (colonnes de brackethive_games, sans id).
	 *
	 * @param array $row       Ligne brute.
	 * @param int   $tid       Tournoi de destination.
	 * @param array $team_map  Correspondance des identifiants d'équipes.
