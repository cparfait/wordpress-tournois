...
class Brackethive_Updater {

	const TRANSIENT = 'brackethive_update_manifest';
	...
	/**
	 * Identifiant du fichier principal (« brackethive/brackethive.php »).
	 *
	 * @return string
	 */
	public static function basename() {
		return plugin_basename( BRACKETHIVE_FILE );
	}
	...
	public static function slug() {
		return dirname( plugin_basename( BRACKETHIVE_FILE ) );
	}
	...
	public static function manifest( $force = false ) {
		$url = Brackethive_Settings::get( 'update_manifest_url' );
	...
	public static function info( $result, $action, $args ) {
		...
		$info                = new stdClass();
		$info->name          = '' !== $manifest['name'] ? $manifest['name'] : 'Brackethive';
		$info->slug          = self::slug();
	...
	public static function verified_download( $reply, $package, $upgrader = null ) {
		...
		if ( ! is_string( $hash ) || ! hash_equals( $manifest['sha256'], strtolower( $hash ) ) ) {
			wp_delete_file( $tmp );
			return new WP_Error(
				'brackethive_bad_checksum',
				__( 'The SHA-256 hash of the downloaded package does not match the one declared in the manifest: installation refused.', 'brackethive' )
			);
		}
