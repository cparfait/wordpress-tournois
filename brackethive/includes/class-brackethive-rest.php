...
class Brackethive_Rest {
	...
	public static function render( $request ) {
		...
		if ( ! in_array( $view, Brackethive_Render::views(), true ) || 'registration' === $view ) {
			return new WP_Error( 'brackethive_bad_view', __( 'Unknown view.', 'brackethive' ), array( 'status' => 400 ) );
		}

		$ref = (string) $request->get_param( 'tournament' );
		$tid = '' !== $ref ? Brackethive_Tournament::resolve( $ref ) : Brackethive_Tournament::current_public();

		if ( ! $tid && 'list' !== $view ) {
			return new WP_Error( 'brackethive_no_tournament', __( 'Tournament not found.', 'brackethive' ), array( 'status' => 404 ) );
		}
	...
	public static function state( $request ) {
		...
		foreach ( $matches as $match ) {
			$out[] = array(
				...
				'status'   => $match['status'],
				'team1'    => (int) $match['team1_id'] ? Brackethive_Data::team_name( (int) $match['team1_id'], $teams ) : Brackethive_Data::source_label( $match['src1'] ),
				'team2'    => (int) $match['team2_id'] ? Brackethive_Data::team_name( (int) $match['team2_id'], $teams ) : Brackethive_Data::source_label( $match['src2'] ),
				'score1'   => (int) $match['score1'],
				'score2'   => (int) $match['score2'],
				'winner'   => (int) $match['winner_id'] ? Brackethive_Data::team_name( (int) $match['winner_id'], $teams ) : '',
			);
		}
		...
		return rest_ensure_response(
			array(
				...
				'game'       => Brackethive_Tournament::get( $tid, 'game_name' ),
				'champion'   => $stats['champion_id'] ? Brackethive_Data::team_name( $stats['champion_id'], $teams ) : '',
				'third'      => $stats['third_id'] ? Brackethive_Data::team_name( $stats['third_id'], $teams ) : '',
				'stats'      => $stats,
				'matches'    => $out,
			)
		);
