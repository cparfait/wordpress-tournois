...
class Brackethive_Standings {
	...
	public static function group( $tournament_id, $group_id, $matches = null, $teams = null ) {
		...
		if ( empty( $rows ) ) {
			foreach ( Brackethive_Data::group_members( $tid, $group_id ) as $id ) {
	...
	protected static function blank( $team_id, $teams ) {
		return array(
			'team_id' => (int) $team_id,
			'name'    => Brackethive_Data::team_name( $team_id, $teams ),
	...
	public static function groups( $tournament_id, $matches = null ) {
		if ( null === $matches ) {
			$matches = Brackethive_Data::get_matches( (int) $tournament_id );
		}
	...
	public static function final_ranking( $tournament_id ) {
		...
		if ( isset( $by_code['GF2'] ) && Brackethive_Data::is_match_needed( $by_code['GF2'], $by_code ) ) {
