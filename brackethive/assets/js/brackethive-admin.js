/* Brackethive — script d'administration */
( function () {
	'use strict';

	function ready() {
		// Passe automatiquement le statut à « Terminé » dès qu'un score décisif est saisi.
		var forms = document.querySelectorAll( '.brackethive-match-form' );
	...
	function adminText( key, fallback ) {
		if ( 'undefined' !== typeof BRACKETHIVE_ADMIN && BRACKETHIVE_ADMIN.i18n && BRACKETHIVE_ADMIN.i18n[ key ] ) {
			return BRACKETHIVE_ADMIN.i18n[ key ];
		}
		return fallback;
	}
	...
	function initCopyButtons() {
		var buttons = document.querySelectorAll( '[data-brackethive-copy]' );

		Array.prototype.forEach.call( buttons, function ( button ) {
			button.addEventListener( 'click', function () {
				var row = button.closest( '.brackethive-link__row' );
	 ...
	 */
	function initWizard() {
		var form = document.querySelector( '[data-brackethive-wizard]' );
		if ( ! form ) {
			return;
		}

		var panels = form.querySelectorAll( '[data-brackethive-step]' );
		var links = form.querySelectorAll( '[data-brackethive-step-link]' );
		var prev = form.querySelector( '[data-brackethive-prev]' );
		var next = form.querySelector( '[data-brackethive-next]' );
		var submit = form.querySelector( '[data-brackethive-submit]' );
		var total = panels.length;
		var current = 1;
		var i18n = {};

		if ( window.BRACKETHIVE_WIZARD_I18N ) {
			i18n = window.BRACKETHIVE_WIZARD_I18N;
		}
		...
		function show( step ) {
			...
			Array.prototype.forEach.call( panels, function ( panel ) {
				panel.hidden = parseInt( panel.getAttribute( 'data-brackethive-step' ), 10 ) !== current;
			} );
			Array.prototype.forEach.call( links, function ( link ) {
				var n = parseInt( link.getAttribute( 'data-brackethive-step-link' ), 10 );
		...
		function validate() {
			var panel = form.querySelector( '[data-brackethive-step="' + current + '"]' );
		...
		function syncFormat() {
			...
			form.querySelectorAll( '.brackethive-choice' ).forEach( function ( card ) {
				var input = card.querySelector( 'input' );
				card.classList.toggle( 'is-selected', !! ( input && input.checked ) );
			} );
			Array.prototype.forEach.call( form.querySelectorAll( '.brackethive-if-groups' ), function ( el ) {
				el.style.display = 'groups' === fmt ? '' : 'none';
			} );
			Array.prototype.forEach.call( form.querySelectorAll( '.brackethive-if-double' ), function ( el ) {
				el.style.display = 'double' === fmt ? '' : 'none';
			} );
			Array.prototype.forEach.call( form.querySelectorAll( '.brackethive-if-not-double' ), function ( el ) {
				el.style.display = 'double' === fmt ? 'none' : '';
			} );
		}
		...
		function set( key, text ) {
			var el = form.querySelector( '[data-brackethive-sum="' + key + '"]' );
			if ( el ) {
				el.textContent = text || '—';
			}
		}
		...
		var mirror = form.querySelector( '[data-brackethive-mirror]' );
		if ( mirror ) {
			mirror.addEventListener( 'change', function () {
				var target = form.querySelector( '[name="' + mirror.getAttribute( 'data-brackethive-mirror' ) + '"]' );
				if ( target ) {
	...
	function initFormatFields() {
		...
		var rows = document.querySelectorAll( '.brackethive-if-groups' );
	...
	function initPreviewFrame() {
		var frame = document.getElementById( 'brackethive-preview-frame' );
		if ( ! frame ) {
			return;
		}
		...
		window.addEventListener( 'message', function ( event ) {
			...
			if ( ! event.data || ! event.data.brackethivePreviewHeight ) {
				return;
			}
			var h = parseInt( event.data.brackethivePreviewHeight, 10 );
			if ( h > 0 && h < 20000 ) {
